...
class Database(object):
    ...
    def __compat_things(self):
        ...
        if self.id_ind.entry_line_format[4:6] == '4s':
            # rev compatibility...
            warnings.warn("Your database is using old rev mechanizm for ID index. You should update that index ("
...
you should check index code.""" % (index.name, ex), RuntimeWarning)
    ...
    def get_many(self, index_name, key=None, limit=-1, offset=0, with_doc=False, with_storage=True, start=None,
                 end=None, **kwargs):
        """
    ...
    def __get_size(self):
        ...
        return sum(
            os.path.getsize(os.path.join(dirpath, filename)) for dirpath, dirnames,
                                                                 filenames in os.walk(self.path) for filename in
            filenames)
